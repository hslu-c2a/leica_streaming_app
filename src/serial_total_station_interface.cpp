...
SerialTSInterface::SerialTSInterface(std::function<void(const double, const double, const double)> locationCallback)
  : serial_port_(*io_context_), TSInterface(locationCallback)
{
}
...
void SerialTSInterface::startReader()
{
  boost::asio::async_read_until(
      serial_port_, readData_, "\r\n",
      std::bind(&SerialTSInterface::readHandler, this, std::placeholders::_1, std::placeholders::_2));
}
...
void SerialTSInterface::write(std::vector<char> command)
{
  boost::asio::async_write(
      ...
      std::bind(&SerialTSInterface::writeHandler, this, std::placeholders::_1, std::placeholders::_2));
}

void SerialTSInterface::writeHandler(const boost::system::error_code& ec, std::size_t bytes_transferred)
{
  if (!ec)
  {
...
void SerialTSInterface::readHandler(const boost::system::error_code& ec, std::size_t bytes_transferred)
{
  if (!ec)
  {
    // Convert streambuf to std::string
    boost::asio::streambuf::const_buffers_type bufs = readData_.data();
    std::string data(boost::asio::buffers_begin(bufs), boost::asio::buffers_begin(bufs) + readData_.size());
    ...
    {  // Forward x, y and z coordinate if location was received
