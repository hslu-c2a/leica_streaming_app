...
TCPTSInterface::TCPTSInterface(std::function<void(const double, const double, const double)> locationCallback)
  : socket_(new boost::asio::ip::tcp::socket(*io_context_)), TSInterface(locationCallback)
{
}
...
void TCPTSInterface::startReader()
{
  boost::asio::async_read_until(
      *socket_, readData_, "\r\n",
      std::bind(&TCPTSInterface::readHandler, this, std::placeholders::_1, std::placeholders::_2));
}
...
void TCPTSInterface::write(std::vector<char> command)
{
  boost::asio::async_write(
      *socket_, boost::asio::buffer(command),
      std::bind(&TCPTSInterface::writeHandler, this, std::placeholders::_1, std::placeholders::_2));
}

void TCPTSInterface::writeHandler(const boost::system::error_code& ec, std::size_t bytes_transferred)
{
  if (!ec)
  {
...
void TCPTSInterface::readHandler(const boost::system::error_code& ec, std::size_t bytes_transferred)
{
  if (!ec)
  {
